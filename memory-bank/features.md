...
| Extração LLM dos PDFs + sinais de alerta | 1, 3 | P3; base Lucas | ✅ | `src/extractor`: PDFs → segurança → brief (≈1/3 do texto) → OpenAI structured output (gpt-5-mini) → `DadosExtraidos`; sinais decidíveis por regra vencem o LLM (decisão 34); validado nos dois processos exemplo; cache em `data/cache/extractor`; `make extrair PASTA=… ARGS="--sem-llm --brief"` mostra o que iria ao LLM sem gastar; `make exemplos-docs` copia os PDFs de `docs/Caso_*/` para `data/exemplos/`; `make bench-extractor` mede a compressão de tokens (números e achados em `dados.md`, relatório em `docs/extractor/benchmark.md`); benchmark com chamadas reais (4 modelos × 2 casos × 3 chamadas, gabarito objetivo, tabelas do deck) em `docs/extractor/benchmark-llm.md`; prompt 2026-09-12.4 com regras de contradição e formato telegráfico (decisão 35); itens que citam subsídio ausente saem; 40 testes com dublê do LLM |
| Análise e minutas em linguagem jurídica | 3 | P3; base Lucas | 🔧 | análise (tese do autor, pontos fortes/fracos com fonte, riscos, contradições petição × subsídios) sai da mesma chamada do LLM que a extração; minutas (proposta, roteiro de defesa, mensagem ao adverso) por template em `extractor/minutas.py` com os valores da recomendação; falta: minutas via LLM e campo próprio para contradições no contrato |
