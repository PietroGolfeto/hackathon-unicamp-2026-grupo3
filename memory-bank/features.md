...
| Extração LLM dos PDFs + sinais de alerta | 1, 3 | P3; base Lucas | ✅ | `src/extractor`: PDFs → segurança → brief (≈1/3 do texto) → OpenAI structured output (gpt-5-mini) → `DadosExtraidos`; sinais decidíveis por regra vencem o LLM (decisão 34); validado nos dois processos exemplo; cache em `data/cache/extractor`; `make extrair PASTA=… ARGS="--sem-llm --brief"` mostra o que iria ao LLM sem gastar; `make exemplos-docs` copia os PDFs de `docs/Caso_*/` para `data/exemplos/`; 32 testes com dublê do LLM |
