...
| Extração LLM dos PDFs + sinais de alerta | 1, 3 | P3; base Lucas | ✅ | `src/extractor`: PDFs → segurança → brief (≈1/3 do texto) → OpenAI structured output → `DadosExtraidos` com sinais (inclui LIVENESS_AUSENTE_CANAL_DIGITAL e DOCUMENTO_SUSPEITO); cache em `data/cache/extractor`; `make extrair PASTA=… ARGS="--sem-llm --brief"` mostra o que iria ao LLM sem gastar; `make exemplos-docs` copia os PDFs de `docs/Caso_*/` para `data/exemplos/`; 32 testes com dublê do LLM |
