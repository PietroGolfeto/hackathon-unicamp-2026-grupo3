...
| Engine v2: três ações por valor esperado (defesa, acordo, instruir), breakeven p* por caso, EVSI por conjunto de subsídios ausentes com probabilidade total, saldo devedor nos dois ramos, IC epistêmico e `decisao_sensivel`; escada de negociação; backtest do engine | 1, 2, 5 | P1 | ✅ | `src/enteros/policy/{engine,negotiation,custos}.py`, `policy.yaml` v2 (cenário `jec`, `evsi_min_pct_causa`, `q_doc_padrao`), `src/enteros/backtest/`, `docs/backtest/`, `make backtest`; campos aditivos em `Recomendacao` (`p_breakeven`, `instrucao`, `analise_subsidios`, `docs_que_fortalecem`, …) |
