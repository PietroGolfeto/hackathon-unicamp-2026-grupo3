...
22. **Dois CSVs sintéticos nossos versionados em `data/exemplos/`, um por consumidor: `sinteticos.csv` (3 mil, schema da planilha; engine e testes) e `sinteticos_processos.csv` (340 com autor/escritório; seed-demo). Pastas `data/exemplos/<numero>/` dos processos reais são ignoradas.** Renomear o do portal foi mais barato que mudar o loader do engine.
...
25. **Na fase 1 coexistem a política de referência do engine (`policy.yaml`, faixas, escada, backtest do deck) e a política operacional da API (`PoliticaParams` versionados no banco, simulação ao vivo).** Convergência: API consome o modelo do engine via adapter (`MODEL_IMPL`) e P1 calibra os defaults dos `PoliticaParams` pelo `policy.yaml` (mapa em `contratos.md`). Sem isso, a demo mostraria um número no deck e outro no painel.
