...
## Fluxos
0. **Engine**: `make data` (xlsx em `data/raw/` ou `sinteticos.csv`) → `make train` grava `models/*.json` → `make backtest` grava `docs/backtest/resumo.{json,md}`, gráficos e o bloco marcado de `docs/relatorio.md`.
1. **Carga**: os três casos da tela do advogado estão versionados em `data/exemplos/<numero>/{autos,subsidios}` (PDFs da Enter — decisão 19), o terceiro derivado do primeiro sem o extrato para cobrir as três saídas do engine; `make exemplos-docs` (`scripts/mocks_advogado.sh`) os regenera a partir de `docs/Caso_*/`. `load-historico` lê os 2 CSVs → merge → scores OOF de P1 (ou stub) → `historico_sentencas`. `ingest` lê `data/exemplos/<numero>/` → flags pelos nomes dos PDFs + UF pelo CNJ → `Extrator.extrair` (cache hit não chama a OpenAI; sem chave e sem cache o ingest para com `ErroConfiguracao`) → scores de P1 (ou stub) → `Extrator.analisar` (mesma chamada) → `processos`. Idempotente; nunca toca `decisoes`.
