...
🔧 `src/web`: Vite + React 19 + Mantine 8 + TanStack Query + react-router 7; login, casos e caso prontos; páginas do gestor em andamento
...
| web | `src/web` | 🔧 | SPA (`src/`): `api/client.ts` (tipos = schemas da API), `auth/useSession.ts`, `lib/format.ts` (BRL, %, datas), `components/{Layout,Badges,Stat}.tsx`, `pages/Login.tsx`, `pages/advogado/{Casos,Caso}.tsx` (cabeçalho com sinais → card da recomendação → análise → documentos → decisão com cronômetro → contato, minutas copiáveis e resultado); `pages/gestor` em andamento. Dev: proxy `/api` → :8000 |
