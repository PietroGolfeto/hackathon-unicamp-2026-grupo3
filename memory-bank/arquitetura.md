...
🔧 `src/extractor` (P3): leitura de PDF com pdf-inspector (OCR local só nas páginas escaneadas) e resumo dos pontos principais com a OpenAI por CLI (`make resumo PDF=...`) e ficha do caso por pasta (`make caso DIR=...`: fatos extraídos pela LLM, números e decisão pelo engine); ainda não plugado no portal
...
| extractor | `src/extractor` | 🔧 | P3, membro do workspace uv (pacote `extractor`): `leitura.py` (`ler_pdf` → markdown por página rotulado `[página N]`; `process_pdf_with_ocr` só nas `pages_needing_ocr`, com PDFium e ONNX Runtime dos wheels pypdfium2/onnxruntime; sem runtime a página fica em `paginas_sem_texto`), `resumo.py` (`resumir_pdf` → `ResumoDocumento`: OpenAI `responses.parse` com saída estruturada, resumo + pontos principais, modelo por `OPENAI_MODEL`, corte em 200 mil caracteres), `caso.py` (`resumir_caso(pasta)` → `FichaCaso`: autos primeiro + subsídios → `ExtracaoCaso` pela OpenAI (canal, assinatura, pedidos, contrato, prova de proveito, indícios, contradições) → `enteros.CaseFeatures` com subsídios por `core.docs.subsidios_por_arquivos` → `engine_padrao().recomendar` → texto Canal/Subsídios/Prova de proveito/Autor/Segmento na base/Decisão), `__main__.py` (CLI: arquivo = resumo, pasta = ficha; texto ou `--json`; carrega o `.env`), `benchmark.py` (`make benchmark-extractor`: modelos × processos exemplo × repetições em paralelo; 20 critérios contra gabarito conferido nos PDFs, decisão do engine, alucinação de documento ausente, latência, tokens e custo pela tabela de preços da OpenAI; JSON e tabela em `data/derived/`). Falta: `DadosExtraidos`, sinais, análise, minutas e o `Extrator` de `EXTRACTOR_IMPL` |
