...
| web | `src/web` | ✅ | SPA (`src/`): `api/client.ts` (tipos = schemas da API), `auth/useSession.ts`, `lib/format.ts` (BRL, %, datas), `components/{Layout,Badges,Stat}.tsx`, `pages/Login.tsx`, `pages/advogado/{Casos,Caso}.tsx` (cabeçalho → card da recomendação com subsídios presentes/ausentes e fatores do modelo → documentos → decisão com cronômetro → resultado; autor, sinais, análise, contato e minutas só aparecem quando P3 os entrega); `pages/gestor/{Painel,Politica,Aprovacoes}.tsx` (números; gráficos são de P5). Dev: proxy `/api` → :8000 |
